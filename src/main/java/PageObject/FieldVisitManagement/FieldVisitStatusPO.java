...
public class FieldVisitStatusPO extends AbstractMethod {
	WebDriver driver;
	private static Logger logger=LogManager.getLogger(FieldVisitStatusPO.class);
	
	public FieldVisitStatusPO(WebDriver driver) {
		super(driver);
	...
	public void validateFieldVisitFromStatusPage(String usrId, String attendancePeriodMonth,
			String attendancePeriodYear, String startDate, String endDate, String scheduleStartTime,
			String scheduleEndTime, String taskName, String location) throws InterruptedException {
		...
		element_dynamicDDSelect(useridtxt, usrId);
		...
		String dt2 = dtAr[0];
		expStatus = stDt1 + " " + scheduleStartTime + " - " + endDt1 + " " + scheduleEndTime + " - " + taskName.toUpperCase() + " - "
		...
		for (WebElement sst : scheduleStatusTable) {
			...
			if (status1.contains(":")) {
				...
				if (status1.equalsIgnoreCase(expStatus)) {
